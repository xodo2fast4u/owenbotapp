...
export function Navbar({ isDark, toggleTheme }: NavbarProps) {
  ...
  const Logo = () => (
    <a href='#' className='flex items-center gap-2'>
      ...
      <span className='font-semibold text text-lg'>Owen</span>
    </a>
  );
        ...
        <div className='flex items-center justify-between h-16'>
          ...
          <div className='hidden md:flex items-center gap-8'>
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className='textMuted hover:textPrimary text-sm font-medium transition-colors'
              >
                {link.label}
              </a>
            ))}
          ...
          <div className='md:hidden flex items-center gap-2'>
            ...
            <Sheet>
              ...
              <SheetContent side='right' className='bg-bg border-l border-border'>
                ...
                <div className='flex flex-col gap-6 mt-8'>
                  {navLinks.map((link) => (
                    <a
                      key={link.href}
                      href={link.href}
                      className='textMuted hover:textPrimary text-lg font-medium transition-colors'
                    >
                      {link.label}
                    </a>
                  ))}
